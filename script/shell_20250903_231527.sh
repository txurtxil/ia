...
generate_openscad_code() {
    ...
    # Pedimos a la IA que devuelva el código en un JSON.
    FULL_PROMPT="Genera solo un objeto JSON válido con una clave 'openscad_code' cuyo valor sea código funcional, completo y válido de OpenSCAD para la siguiente descripción. No incluyas texto extra, explicaciones ni bloques de código markdown.
    $prompt_content"
    ...
    # Procesamos la respuesta en un solo paso para evitar errores.
    # Extraemos el texto de la respuesta, eliminamos los marcadores de markdown y luego extraemos el código JSON.
    code_content=$(jq -r '.candidates[0].content.parts[0].text' "$tmp_response" 2>/dev/null | sed 's/^```json//' | sed 's/```$//' | jq -r '.openscad_code' 2>/dev/null)
    ...
    if [ -z "$code_content" ] || [ "$code_content" == "null" ]; then
        ...
        cat "$tmp_prompt"
        echo "La respuesta de la IA fue (posiblemente un JSON mal formado):"
        cat "$tmp_response"
...
generate_freecad_code() {
    ...
    # Pedimos a la IA que devuelva el código en un JSON.
    FULL_PROMPT="Genera solo un objeto JSON válido con una clave 'freecad_code' cuyo valor sea código funcional, completo y válido de Python para FreeCAD basado en la siguiente descripción. No incluyas texto extra, explicaciones ni bloques de código markdown.
    $prompt_content"
    ...
    # Procesamos la respuesta de la misma manera que en la función de OpenSCAD.
    code_content=$(jq -r '.candidates[0].content.parts[0].text' "$tmp_response" 2>/dev/null | sed 's/^```json//' | sed 's/```$//' | jq -r '.freecad_code' 2>/dev/null)
    ...
    echo "$code_content" > "$HOME/modelo.py"
    rm "$tmp_prompt" "$tmp_response"
...
5) Generar OpenSCAD con IA
6) Generar FreeCAD con IA
0) Volver
=================="
