...
generate_freecad_code() {
    ...
    # Pedimos a la IA que devuelva solo el código.
    FULL_PROMPT="Genera un script de Python completo y válido para FreeCAD basado en la siguiente descripción. El script debe ser totalmente funcional y guardar el resultado como un archivo STL con la ruta completa '$DL/modelo.stl'. Utiliza solo operaciones booleanas y formas básicas (cajas, cilindros) para construir geometrías complejas. No incluyas texto extra, explicaciones, JSON ni bloques de código markdown. Solo devuelve el código.
    $prompt_content"
