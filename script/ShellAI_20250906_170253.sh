...
# ===================== CREAR PLANTILLAS (FORZAR RECREACIÓN) =====================
# Borramos todas las plantillas antiguas
rm -f "$TEMPLATES_DIR"/*.scad.prompt
rm -f "$TEMPLATES_DIR"/*.py.prompt

# Creamos las plantillas desde cero
create_default_templates() {
    # Plantilla: Caja para OpenSCAD
    cat > "$TEMPLATES_DIR/caja_openscad.scad.prompt" << 'EOF'
...
EOF
    ...
    # Plantilla: Engranaje para OpenSCAD
...
EOF
    ...
    # Plantilla: Soporte en L para FreeCAD
...
EOF
    ...
    # Plantilla: Caja Básica para FreeCAD
    ...
    case $choice in
        ...
        2)
            ...
            for i in "${!templates[@]}"; do
                ...
                # Mostrar SOLO nombres amigables
                case "$template_name" in
                    "caja_openscad") display_name="Caja Rectangular" ;;
                    "engranaje_openscad") display_name="Engranaje" ;;
                    *) continue ;; # Ignorar cualquier otro archivo
                esac
...
upload_shell_script() {
    ...
    fecha=$(date +%Y%m%d_%H%M%S)
    archivo="script/ShellAI_${fecha}.sh" # <-- CAMBIO CLAVE: Nuevo formato de nombre
...
EOF
    ...
    echo "✅ Script subido como: $archivo"
    read -n1 -s -r -p "Pulsa cualquier tecla..."
}
